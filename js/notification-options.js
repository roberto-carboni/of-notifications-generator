...
const stickySwitch = document.getElementById("is-sticky-toast");
...
const initListeners = () => {
  ...
  indicatorType.addEventListener("ionChange", async () => {
    if(indicatorType.value === '') {
      stickySwitch.checked = false;
      stickySwitch.disabled = true;
    } else {
      stickySwitch.disabled = false;
    }
    await buildCodeSample();
  });

  stickySwitch.addEventListener("ionChange",  buildCodeSample);
